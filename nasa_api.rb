...
def request(address, api_key)
    ...
    body = "https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=1000&api_key=DEMO_KEY&page=1&"
    body = body["photos"]

#crear un metodo llamado buid_web_page
def buid_web_page(hash)#que reciba en hash de respuesta
    #con todos los datos y construya una pagina web
    #se evalúa la pagina creada y tiene que tener formato

    html = "<html>\n<title>Api Nasa</title>\n<head>\n</head>\n<body>\n<h2>Api Nasa</h2>\n<ul>\n"
    hash.map {|img| html += "\t<li><img src=\"#{img["img_src"]}\"></li>\n"}
    html += "</ul>\n</body>\n</html>"
...
def photos_count(hash)#que reciba un hash de respuesta
    ...
    hash = request("https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=1000&api_key=DEMO_KEY&page=1")[0..25]#limitamos los restultados a 25
